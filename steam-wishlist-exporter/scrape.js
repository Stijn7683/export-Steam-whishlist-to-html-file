(async function () {
  try {
    ...
    if (!container) {
      alert("Wishlist container not found.");
      return;
    }
    ...
    const discountedApps = Array.from(clone.querySelectorAll("a.oVvbc-NOBF8-")).map(a => {
      ...
      return null;
        ...
        ["mouseover", "S4P-Tu6KIaQ-"].forEach(evt =>
            chrome.runtime.sendMessage({type: "log", message: `did something`})
        );
        
    }

    
    container.querySelectorAll(".oVvbc-NOBF8-").forEach(row => {
      triggerHover(row);
      chrome.runtime.sendMessage({type: "log", message: `hover`});
      row.dataset.hoverTriggered = "true"; 
    });
    ...
    // ---------- 3a. Remove date-added + remove-button rows ----------
    ...
    // ---------- 3c. Replace old buttons with "View in Steam" ----------
...
*/
    // debug: how many elements in the clone have the attribute
    console.log("[scrape.js] elements with data-discount-end:", clone.querySelectorAll("[data-discount-end]").length);
    


    // ---------- 6. Build final standalone file ----------
    const clientScript = `
